...
# separate all objects from scene into cubes and main_object
cubes = []
for _3d_object in bpy.data.objects:
    # criteria: object name containts the string 'Cube'
    if 'Cube' in _3d_object.name:
        cubes.append(_3d_object)
    else:
        main_object = _3d_object
        print ('Main object' + main_object.name)

print (cubes)


for cube in cubes:
    print ("Entering cube %s" % cube.name)
    
    intersect = bmesh_check_intersect_objects(main_object, cube)
    if not intersect:
        print ("Cube %s at location %s does NOT intersect object. Will be deleted" % (cube.name, cube.location))
        ...
        print ("Cube %s at location %s intersects object. Will be kept" % (cube.name, cube.location))
        
    print ("Exit cube %s" % cube.name)